...
directory = os.path.expanduser("~/Dropbox/local_miscellaneous_data/test_data/ENCODE_poster/ideas_overlap_encode")
...
ideas_hepg2_file= os.path.expanduser("~/Dropbox/local_miscellaneous_data/genomic_models/hepg2_ideas")
ideas_hepg2_file= os.path.expanduser("~/Dropbox/local_miscellaneous_data/genomic_models/head_200_hepg2_ideas")
...
JSON_dict_file = os.path.expanduser(os.path.join(directory,"ENCODE_ideas_JSON_1.txt"))
...
file_name_list = [key+".txt" for key in master_ideas_dict.iterkeys()]
...
for key,value in master_ideas_dict.iteritems():
	...
	# Original A and B entries plus the no. of base pairs overlap b/w 2 features.
	#bed_intersect = peak_file.intersect(ideas_loci_bed_file, wo = True)
	...
	if intersect_count >= 1:
		...
		master_ideas_dict[key]["As_overlap_list"][0] = master_ideas_dict[key]["As_overlap_list"][0].drop_duplicates()
		...
		master_ideas_dict[key]["Bs_overlap_list"][0] = master_ideas_dict[key]["Bs_overlap_list"][0].drop_duplicates()
		...
		master_ideas_dict[key]["As_overlap_list"][0].to_csv(outfile_path_prefix[0] + "_with_A_overlap.bed")
...
out_file = directory + "/" + "intersectbed_counts_with_ideas.txt"
