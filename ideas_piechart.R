...
# Translates all rgb codes to hex codes:
...
cols <- sapply(strsplit(as.character(read_file$rgb_code), ","), function(x) {
  rgb(x[1], x[2], x[3], m=255) #rgb(rval, gval, bval, max)
}) 
...
output_file_name <- paste0("~/Dropbox", "/", tf_name, "_ideas_piechart_col.pdf")				
...
pie_plot <- bar + coord_polar("y", start=0) + 
			...
			#scale_fill_manual(name="IDEAS States", values = custom_col, na.value="grey50") + 
